...
public class Derive {
    ...
    private final ArrayList<String> list; // Contient la liste des expression
    private final ArrayList<String> listDerive; // Contient la liste des parties dérivé
    ...
    private void getSymboleFromExpression(){
        ...
        if(split.length > 1 && split[0].length() > 0 && split[1].length() > 0){
            ...
            if(part1.startsWith("der(") && part1.endsWith(")")){
                ...
                if(part1.length() == 1){ symbole = part1; }
                else symbole = "x";
            }
            ...
            if(stringContainsComplexe(f)){
                if(f.startsWith("(") && f.endsWith(")")){
                    ...
                    if(f.contains("/") && !f.contains("*")){ // u*v -> [((u' * v) - (u * v')) / v²]
                        ...
                        if(split[0].isEmpty() || split[1].isEmpty()){
                            ...
                            u      = split[0];
                            v      = split[1];
                            dr     = new Derive(u, symbole);
                            uprime = dr.getResult();
                            dr     = new Derive(v, symbole);
                            vprime = dr.getResult();
                ...
                try {
                    ...
                    if(split[0].isEmpty()){ coeff = "1"; }
                        ...
                        if(split[0].contains("/")){
                            coeff = split[0];
                        } else throw new Exception("Erreur syntaxe division : (a/b)");
